...
//    }
  ...
  private int sampleTopicZ(int m, int n) {
    ...
    //
    int newTopic = 0;
    float topicCursor = (float) (Math.random() * topicsSum);
    for (int k = 0; k < K; k++) {
      topicCursor -= p[k];
      if (topicCursor <= 0) {
        newTopic = k;
...
//        }
    ...
    for(int i = 0 ; i < M ; i++) {
      ...
      if(index >= docSet.filesNumMark.get(1)) break;
  ...
  public Community getCommunity(Documents docSet) {
    double thresholdForER = 0.02 ;
    double thresholdForEE = 1 / 1000;
    List<Integer> CF; // followers for community
    List<Integer> CG; // followees for community 
    
    thresholdForER = threshold;
