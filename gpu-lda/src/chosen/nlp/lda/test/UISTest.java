...
//    scanner.close();
    ...
    if(args.length > 1 && args[1].equals("read") ) {
      ...
      lda.threshold = Double.parseDouble(args[2]);
      System.out.println("enter threshold:" + lda.threshold);
      lda.saveCommunity();
    } else {
      try {
        int division = Integer.parseInt(args[1]);
        System.out.println("How many Interest Topic?" + division);
        double threshold = Double.parseDouble(args[2]);
        System.out.println("enter threshold:" + threshold);
        double weigh = Double.parseDouble(args[3]);
        System.out.println("enter weigh:" + weigh);
